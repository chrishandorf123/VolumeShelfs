...
TRAIL TAB — a quiet, open reconstruction of paid "trail + CALL/PUT" overlays (à la IncomeSharks' invite-only Sharks Stock Trader), on the symbol loaded in Explore. Two SuperTrends (tight 10/×1.5 + loose 11/×2.5) draw trail dots ONLY while both agree on a side (no dots = stand aside). A LONG/SHORT signal fires only when agreement flips to a NEW side and, within a 5-bar grace window, the close is on the signal's side of the 20-bar EMA and OBV's 10-bar direction doesn't contradict — one signal per regime, max; gated-out flips are counted as "skipped". Each signal carries a fixed plan (stop = the loose trail at entry, target = entry ± 2R) and every past signal is replayed bar-by-bar (win / stopped / flipped out — stop counted first when one bar spans both) into win rate, expectancy (avg R) and total R. Honesty notes baked in: it is NOT IncomeSharks' closed-source script and won't match their signals; the replay is in-sample (a sanity check, not proof of edge); educational only — the Final Call and discipline guard stay the referee.
